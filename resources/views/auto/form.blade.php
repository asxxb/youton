...
@if(session('video'))
    ...
    <form action="{{ route('youtube.upload') }}" method="POST">
        @csrf
        <input type="hidden" name="video_path" value="{{ session('video') }}">
        <input type="hidden" name="title" value="My Youton Short">
        <input type="hidden" name="description" value="Auto-generated by Youton">

        <button class="btn" style="margin-top: 15px;">🚀 Push to YouTube</button>
    </form>
@endif
